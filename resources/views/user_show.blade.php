...
<section class="text-justify">
    <div class="title">
        <h2 class="blue text-uppercase font-title">Datos del usuario - {{ $show->username }}</h2>
    </div>
    ...
    <div class="col-sm-12 col-md-11 m-auto">
        ...
        <div class="dos-grid">
            ...
            <div class="img-user center text-light text-center">      
                <div>
                    ...
                    <div class="row">
                        {{-- Fecha actual... --}}
                        <form action="{{ route('user.update', $show->id) }}" method="POST" class="col-sm-12 p-1">
                            @csrf
                            <input type="hidden" name="_method" value="DELETE">
                            <button type='submit' onclick="return confirm('¿Seguro que deseas eliminarlo?')" class="btn btn-outline-light btn-block">Eliminar</button>
                        </form>

                        <a href="{{ route('user.index') }}" class="col-sm-12 p-1">
        ...
        <div class="my-5">
            <div class="alert alert-secondary" role="alert">Listado de todas las reservas que ha realizado el usuario en <b>nuestro club deportivo</b>.</div>
            <div class="col-md-5 col-sm-12 m-auto p-0">
                {{-- Obtenemos las pistas del club --}}
                @foreach (Auth::user()->club->tracks as $item)
                    {{-- Reservas del usuario --}}
                    @foreach ($show->reservations as $reserve)
                        {{-- Muestrame las reservas que ha realizado en el club --}}
                        @if ($reserve->club_track_id == $item->id)
                            <a href="{{ route('admin.reserve', $reserve->id) }}" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center ">
                                {{ $reserve->start }}
                                <span class="badge badge-danger badge-pill ml-2  {{ ($reserve->pivot->status == 2) ? 'pending-reserve pending' : (($reserve->pivot->status == 1) ? 'reserved success' : 'cancelled-reserve cancelled') }}">{{ ($reserve->pivot->status == 2) ? 'pendiente' : (($reserve->pivot->status == 1) ? 'reservada' : 'cancelada') }}</span>
                            </a>
                        @endif
                    @endforeach
                @endforeach                
