...
<section>
    <div class="content-info">
        <div class="title-admin p-4 text-uppercase">
            <h2>Gestión de pistas - {{ Auth::user()->club->name }}</h2>
        </div>

        <div class="mb-5">
            <p>Gestionar las pistas de pádel nunca fue tan facil, en la siguiente tabla puedes ver un listado de las pistas que dispone el club deportivo <b>{{ Auth::user()->club->name }}</b>.</p>

            <div class="main-step">
                <div class="alert alert-primary mt-3" role="alert">No dudes en añadir una nueva pista en la aplicación...</div>
                <a href="{{ route('track.create') }}" class="btn-club">
                    <button class="btn-club btn btn-outline-info  btn-lg">Registrar pista</button>
                </a>
            </div>
        </div>
